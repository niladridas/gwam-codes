...
compTorqueController<NSTATES, NR, InputType1, InputType2, OutputType, MathTraits>::compTorqueController(const std::string& sysName) :
	...
	wamJVIn(this), posErrorIn(this), posErrorDotIn(this), refAccIn(this), compConOp(this, &compConOpValue),pEout(this, &pEoutVal), vEout(this, &vEoutVal), refOut(this, &refOutVal), ctorque(0.0), rCount(0),
	x(Eigen::Matrix<double, NSTATES, 1>()),	err_states_wam(Eigen::Matrix<double, NSTATES, 1>()), u(Eigen::Matrix<double, NSTATES/2, 1>()),qDesDD(Eigen::Matrix<double, NSTATES/2, 1>()),
	...
	T_s(0.0), currentJp(0.0), prevJp(0.0), currentJv(0.0),intError(0.0), intErrorLimit(0.0),	error_jp(0.0), error_1_jp(0.0), error_jv(0.0), error_1_jv(0.0),
	kp(0.0), ki(0.0), kd(0.0),pE(0.0), vE(0.0), refacc(0.0), controlSignal(0.0), controlSignalLimit(0.0)
	{
...
Controller<InputType1, OutputType>(sysName),
	...
	setCompTorqeGains("./FuzzParams/compTorqueGain.txt");
	qDesDD.fill(0.0);
	u.fill(0.0);
	x.fill(0.0);
	err_states_wam.fill(0.0);
...
//		error_jp = MT::sub(refJp, currentJp);
		error_jp = posErrorIn.getValue();
		vE = posErrorDotIn.getValue();
		refacc = refAccIn.getValue();
