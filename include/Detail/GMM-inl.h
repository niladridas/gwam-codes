...
GMM::GMM(double Rho_0, double K) :
		priors(Eigen::VectorXd()), mean(Eigen::MatrixXd()), sigma(
	...
	Sam::initEigenMat<double>(P_total_data,
			Sam::readFile<double>("data/P_total.txt"));
	...
	if (Sam::readConfigFile("data/sysconfig.txt", allData_inp)) {
		...
		if (!Sam::findValueFromMap<size_t>(allData_inp, "num_lyp_asym_copont",
				num_lyp_asym_copont)) {
			exit (EXIT_FAILURE);
		}
		std::vector<double> xi;
		if (!Sam::findValueFromMap<double>(allData_inp, "xi_star_training",
				xi)) {
			exit (EXIT_FAILURE);
		}
...
double GMM::gausspdf(const Eigen::VectorXd& input, const Eigen::VectorXd& Mean,
	...
	double exponential_part = (-0.5
			* (((input - Mean).transpose()) * (Sigma.inverse()))
			* (input - Mean))(0, 0);
...
void GMM::f_estimate(const Eigen::VectorXd& input) {
	...
	for (k = 0; k < num_inp * num_priors - num_inp; k = k + num_inp) {
		A_matrix.block(k, 0, num_inp, num_inp) = sigma_output_input.block(k, 0,
				num_inp, num_inp)
...
void GMM::calculate_gradient(const Eigen::VectorXd& input) {
	...
	if (num_lyp_asym_copont == 0) {
		...
		grad_val = (P_0_data + P_0_data.transpose()) * (input - xistar_trn);
		...
		for (size_t i = 0; i < num_lyp_asym_copont; i++) {
			float tmp_argument = ((input.transpose()-xistar_trn.transpose() )
					* P_total_data.block(i * input_size, 0, input_size,
							input_size)
					* (input - xistar_trn
							- Mu.block(i * input_size, 0, input_size, 0)))(0,
					0);
			if (tmp_argument >= 0) {
				grad_part_2 =
						grad_part_2
...
Eigen::VectorXd GMM::control_input(const Eigen::VectorXd& input) {
	...
	float tmp_value1 = ((grad_val.transpose() * f_hat)(0, 0)
			+ (RHO_0 * (1 - exp(-K_ * input.norm()))));
	if (tmp_value1 <= 0 && input != xistar_trn) {   //changes made here 31.12.2014
		control_input.Zero(input_size);
	} else if (input == xistar_trn) {
		control_input = -f_hat;
	} else {
		control_input = -grad_val * tmp_value1
				/ (grad_val.transpose() * grad_val)(0, 0);
	}
