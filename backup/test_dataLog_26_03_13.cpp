...
int wam_main(int argc, char** argv, ProductManager& pm, systems::Wam<DOF>& wam) {
	...
	typedef  typename systems::myDataProcessing <DOF,NS>::criticTorque_type criticTorque_type;
	srand(time(NULL));
	float ThMax1, ThMax3;
	float ThMin1, ThMin3;
	ThMax1 = 1.5;
	...
	criticTorque_type checkDimCritic(2.0);
	std::cout<<"data in critic type: "<<checkDimCritic<<std::endl;
...
//	connect(wam.jpController.controlOutput, dp.jtInput);
	connect(criticon.controlOutput, dp.jtInput);
	connect(criticon.criticconOp, dp.test_torqueIP);
//	connect(critico, dp.jtInput);
//	connect(wam.jtSum.output, dp.jtSumInput);
	...
	systems::TupleGrouper<double, jp_type, criticTorque_type, double > tg;
//	systems::TupleGrouper<double, vecJp_type, jt_type, jt_type > tg;


	connect(time.output, tg.template getInput<0>());
	connect(dp.jpOutput, tg.template getInput<1>());
//	connect(dp.stateOutput, tg.template getInput<2>());
	connect(dp.test_torqueOP, tg.template getInput<2>());
...
//	typedef boost::tuple<double, jp_type> tuple_type;
	...
	typedef boost::tuple<double, jp_type, criticTorque_type, double> tuple_type;
//	typedef boost::tuple<double, vecJp_type, jt_type, jt_type> tuple_type;
	...
	systems::PeriodicDataLogger<tuple_type> logger(
			...
			for (size_t i=0; i<1; i++){
				...
				jp[1] = -1.0;//(ThMin1 + (ThMax1 - ThMin1) * float(rand())/RAND_MAX);
				jp[3] = 2.0;//(ThMin3 + (ThMax3 - ThMin3) * float(rand())/RAND_MAX);
