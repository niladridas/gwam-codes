...
myController<NSTATES, InputType1, InputType2, OutputType, MathTraits>::myController(const std::string& sysName) :
	...
	wamJVIn(this), criticconOp(this, &criticconOpValue),//wamJVref(this),
	critic("param.txt", "Wmatrices.txt", "center.txt"),NU(NSTATES/2),
	T_s(0.0), error_jp(0.0), error_1_jp(0.0),currentJp(0.0), error_jv(0.0), error_1_jv(0.0),currentJv(0.0),intError(0.0),intErrorLimit(0.0),
...
void myController<NSTATES, InputType1, InputType2, OutputType, MathTraits>::operate()
{
	typedef MathTraits MT;
		...
		dyn.computeModel(this->feedbackInput.getValue(), wamJVIn.getValue());
		gx_wam=dyn.getGx();
		gth[0] = 0;
		gth[1] = 0;
