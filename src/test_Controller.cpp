...
//	typedef typename systems::myController<NSTATES, NR, jp_type, jv_type, jt_type>::gx_type gx_type;
	...
	typedef systems::TupleGrouper<double, jp_type, jv_type,  jt_type, unitless_type, unitless_type, compTorque_type> tg_type;
...
//	typedef typename systems::virtualSensor<DOF, NSTATES/2>::jv2DOF_type jv2DOF_type;
	...
	typedef systems::myController<NSTATES, NR, jp_type, jv_type, jt_type> criticController_type;
	srand(time(NULL));
	float ThMax1, ThMax3;
	float ThMin1, ThMin3;
	ThMax1 = 1.5;
	...
	criticController_type criticon(setting["joint_position_control"]);
...
//	systems::TupleGrouper<double, jp_type, jt_type, criticTorque_type, unitless_type, unitless_type, vec_type,  vec_type, unitless_type, gx_type > tg;
	...
	systems::connect(wam.jvFilter.output, criticon.wamJVIn);
	systems::connect(mg.BmatOutput2J,criticon.bMat2Dinp);
...
//	connect(compCon.refOut, tg.template getInput<1>());
	...
	connect(compCon.compConOp, tg.template getInput<6>());
...
//	typedef boost::tuple<double, jp_type, jp_type, jp_type, jp_type, compTorUless_type> tuple_type;
	typedef boost::tuple<double, jp_type, jv_type,  jt_type, unitless_type, unitless_type, compTorque_type> tuple_type;
...
//						std::cout<<"moving to: "<<jp<<std::endl;
				time.setOutput(0.0);
				 ...
				 jp_type sp(wam.getJointPositions()), hp(wam.getHomePosition());
				      ...
				      wam.moveTo(jp);
//				      myUtilWAMwrapper<jp_type, criticController_type, tg_type, DOF> wamRap2(&criticon, &wam, &tg, &time);
