...
//  typedef typename Sam::massMatrixHolder<DOF, NSTATES/2>::bu2D_type bu2D_type;
//  typedef typename Sam::massMatrixHolder<DOF, NSTATES/2>::bu_type bu_type;
  typedef systems::compTorqueController<NSTATES, NR, jp_type, jv_type, jt_type> compController_type;
  typedef systems::TupleGrouper<double, jp_type, jv_type, jt_type, ja_type> tg_type;
  ...
  // MassMatrix System
  Sam::massMatrixHolder<DOF, NSTATES/2> mh(pm.getExecutionManager(),pm.getConfig().lookup(pm.getWamDefaultConfigPath()),
...
//  systems::connect(compCon.controlOutput, mh.uInput);
  pm.getExecutionManager()->startManaging(mh);
...
//  systems::connect(mh.BuOutput, tg.template getInput<6>());
  typedef boost::tuple<double, jp_type, jv_type, jt_type, ja_type> tuple_type;
